...
% union
{
...
} % token<str_value> ID % token<str_value> C_STR % token<int_value> C_INT % token<bool_value> TRUE FALSE % token<float_value> C_FLOAT
    ...
    % left ADD SUB MUL DIV MOD % left BT ST SET BET EQL NEQ % left AND OR NOT

    % type<id_type> type_define % type<value> const_expression int_expression bool_expression expression % type<value> components literal_constant var_refer arr_refer
...
program_contents : program_content program_contents | program_content;

program_content : va_declaration | fun_declaration | proc_declaration;
...
va_declaration : const_declaration | var_declaration | array_declaration;
...
formal_argument_list : formal_argument COMMA formal_argument_list | formal_argument;
...
// Statements
statement : simple_statement | condition_statement | loop_statement | proc_invoc;
...
| PRINT LB expression RB | PRINT expression | PRINTLN LB expression RB | PRINTLN expression | RETURN ID | RETURN | RETURN expression;
...
block_content : va_declaration | statement;

// Conditional
condition_statement : IF LB bool_expression RB block_or_simple_statement | IF LB bool_expression RB block_or_simple_statement ELSE block_or_simple_statement;

// Loop
loop_statement : WHILE LB bool_expression RB block_or_simple_statement | FOR LB ID num DOT DOT num RB block_or_simple_statement;

block_or_simple_statement : block | simple_statement;
...
comma_separated_expressions : expression COMMA comma_separated_expressions | expression;
    ...
    int main(int argc, char **argv)
{
    ...
    jasm.open(jasm_target_path, ios::out | ios::trunc);
    /* if(yyparse() == 1){
